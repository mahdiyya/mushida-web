      ...
      <div class="row pagination">
        ...
        <div v-for="index in totalpages" :key="index">
          <template v-if="index >= startpage && index <= parsepage">
            <NuxtLink v-if="index === 1" to="/berita/artikel/" class="index">{{
              index
            }}</NuxtLink>
            <NuxtLink v-else :to="`/berita/artikel/${index}`" class="index">
              {{ index }}
            </NuxtLink>
          </template>
        </div>
  ...
  async fetch({ store }) {
    try {
      const res = await axios.get(
        'https://admin.mushida.org/wp-json/wp/v2/posts?categories=8&per_page=9',
      )
