  ...
  <section>
    <div class="section-container p-t-0">
      ...
      <div class="row pagination">
        <NuxtLink
          v-if="prevpage > 0"
          :to="`/berita/internasional/${theprevpage}`"
          class="index"
        ...
        <NuxtLink
          ...
          :to="`/berita/internasional/${nextpage}`"
          class="index"
  ...
  async fetch({ store }) {
    try {
      const res = await axios.get(
        'https://admin.mushida.org/wp-json/wp/v2/posts?categories=27&per_page=9',
      )
