  ...
  <section>
    <div class="section-container p-t-0">
      ...
      <div class="row pagination">
        <NuxtLink
          v-if="prevpage > 0"
          :to="`/berita/parenting/${theprevpage}`"
          class="index"
        ...
        <div v-for="index in totalpages" :key="index">
          <template v-if="index >= startpage && index <= parsepage">
            <NuxtLink
              ...
              >{{ index }}</NuxtLink
            >
            <NuxtLink v-else :to="`/berita/parenting/${index}`" class="index">
              {{ index }}
            </NuxtLink>
          </template>
        </div>
  ...
  async fetch({ store }) {
    try {
      const res = await axios.get(
        'https://admin.mushida.org/wp-json/wp/v2/posts?categories=23&per_page=9',
      )
